...
from impax_ifire.util.exceptions import iFireWarning
...
class MyLogger(AstropyLogger):
    ...
    def _showwarning(self, *args, **kwargs):
        ...
        if not isinstance(args[0], iFireWarning):
        ...
        # be displayed.
        if type(warning) not in (iFireWarning,):
...
def _init_log(config=None):
    ...
    try:
        log = logging.getLogger("impax_ifire")
